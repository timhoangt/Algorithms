...
******************************************************/
   ...
   public static Node ReturnKthSmallestElement(int k, Node p)
   {
	...
	// Feel free to change the return type, parameters
		if (size(p) - size(p.right) == k){ //if the place of the node is equal to k
			return p; //return the value
		}
		else if (size(p) - size(p.right) > k){//if the place of the node is greater than k
			return ReturnKthSmallestElement(k, p.left);//check the place of the left subtree
		}
		else //if (size(p) - size(p.right) < k){//if the place of the node is less than k
		{
         return ReturnKthSmallestElement(k-size(p.left)-1, p.right);//check the place of the right subtree subtracting the value of the root node
		}
      //return null;
   }
